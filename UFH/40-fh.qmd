...
pak_list <- c("sf", "data.table", "tidyverse", "car", 
              "sae", "survey", "spdep", "knitr", "MASS", 
              "caret", "purrr", "pins", "gt",  "scales",
              "viridis", "emdi", "rlang", "dplyr", "ggplot2")
...
We will show below the use of the `fh()` function of the R package `emdi` [@Harmening2023] which computes the EBLUPs and their MSE estimates of the standard FH model and several extensions of it, among others it allows for the application of transformations. Because the poverty rate is a ratio, it might be helpful to apply the arcsin transformation to guarantee that the results lie between 0 and 1.

\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_ <br> \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_ <br> *A short digression on the arcsine transformation:* <br> To estimate ratios like the poverty rate, it might be helpful to apply the arcsin transformation to guarantee that the results lie between 0 and 1 [@Casas2016; @Schmid2017]. The direct estimates and the sampling variances are transformed as suggested by @Jiang2001: \begin{equation*}
\hat{\theta}_{d}^{\text{Dir*arcsin}} 
= \sin^{-1}\!\left(\sqrt{\hat{\theta}_{d}^{\text{Dir}}}\right),
\qquad
\operatorname{var}\!\left(\hat{\theta}_{d}^{\text{Dir*arcsin}}\right) 
= \frac{1}{4 \tilde{n}_d},
\end{equation*} where the $\text{*arcsin}$ stands for the the arcsin transformed scale and $\tilde{n}_i$ for the effective sample size. The effective sample size equals the sample size of each area divided by the design effect. The design effect is the ratio of the variance considering the sampling design to the variance estimated under simple random sampling. The FH model is then estimated on the transformed scale. If needed, the results are limited to $[0, \pi / 2]$ to ensure results between 0 and 1. To derive the final estimates on the original scale, the obtained estimation results must be back-transformed. We use a bias-corrected back-transformation \[@Sugawasa2017; @Hadam2020\]: \begin{equation*}
    \hat{\theta}^{\text{FH}}_{d} = \int_{- \infty}^{\infty} \sin^2 (t) 
    \frac{1}{2 \pi \frac{\hat{\sigma}^2_u \sigma^2_{e_d}}{\hat{\sigma}^2_u + 
            \sigma^2_{e_d}}} \exp \left( - \frac{\left(t - \hat{\theta}^{\text{FH*arcsin}}_d 
        \right)^2}{2 \frac{\hat{\sigma}^2_u \sigma^2_{e_d}}{\hat{\sigma}^2_u + 
            \sigma^2_{e_d}}} \right) dt. 
\end{equation*} \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_ <br> \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_ <br>
...
For this exercise, we use a random 30% sample of the `incomedata` to estimate the poverty rates. For the univariate case, we use the income variable from 2012.
...
`survey_dt` contains our target area identifiers (`provlab`, `prov`), the weight variable `weight`, the cluster id i.e. enumeration area or psu, `ea_id`, the year, `year`, the income variable `income` and the poverty line `povline`. In this example, `survey_dt` of class `data.frame` is at the level of the individual. It could also be at the level of the household if poverty lines are evaluated at that level.
...
survey_dt <- survey_dt |>
  filter(!!sym(year_var) == singleyear)
...
## computation of direct estimates and their variances (the poverty line is 
...
In theory, the FH model assumes that the variances are known. However, in practical applications they need to be estimated from the unit-level sample data. A further concern is that highly heterogeneous sampling variances can cause instability in the small area predictions. In some areas with a very small sample size, it may occur, that the individual data only consists of zeros and ones, resulting in a direct estimate of zero or one and a direct variance of zero. A strategy to stabilize the variance is to apply a smoothing method. <br> A quick inspection of the preceding results will show some provinces contain low sample sizes which sometimes result in extreme value poverty rates and hence 0 variance. To avoid this, we will show you how to apply the variance smoothing method suggested by @you2023application. Please see the code and Roxygen comments explaining the use of the `varsmoothie_king()` function which computes smoothed variances. In case, the arcsin transformation will be applied, the variance smoothing described here is not necessary, since the arcsin transformation works variance stabilizing itself. When applying the arcsin transformation, the direct variances are automatically set to 1/(4\*effective sampling size) when using the `fh` function of package `emdi`. In this case, we set 0 variance areas to out-of-sample and for the final estimation results only the synthetic part of the FH model is used.
...
```{r}
## set zero variance to OOS (only when using the arcsin transformation)
...
Besides theoretical considerations regarding which auxiliary variables should be included in the model, the selection of the optimal model should be guided by information criteria such as the Akaike or Bayesian information criterion (AIC, BIC). In many applications, variable selection is performed using linear regression-based techniques. In contrast, the `emdi` package provides AIC, BIC, the Kullback information criterion (KIC), as well as their bootstrap and bias-corrected versions (AICc, AICb1, AICb2, KICc, KICb1, KICb2), which were specifically developed for FH models by @Marhuenda2014. Moreover, `emdi` enables a stepwise variable selection procedure based on the chosen information criterion, analogous to the `step` function for `lm` models in the `stats` package.
...
We apply the function to select the variables. Required inputs: data set, character vector containing the set of auxiliary variables, name of y variable, a correlation threshold between 0 and 1, name of information criterion and name of direct variance variable. In case, a transformation should be applied, "arcsin" as transformation and name of variable that contains the effective sample size are also required.
...
#   vardir = "vardir", combined_data = fh_dt, domains = "Domain",
#   method = "ml", MSE = TRUE) 
```
...
fh_bench <- bench(fh_model,
                  benchmark = benchmark_value,
                  share = fh_dt$ratio_n, 
                  type = "ratio", 
                  MSE = TRUE, B = 200, seed = 123,
                  overwrite = TRUE)

head(fh_bench$ind)
head(fh_bench$MSE)
compare_plot(fh_bench, MSE = TRUE, CV = TRUE)
```
...
pov_fh <- pov_fh |>
  ...
  mutate(!!sym(year_var) := singleyear)

write.csv(pov_fh, "data/pov_fh.csv")
```
...
domain_ord <- match(shp_dt[[area_vars[1]]], fh_bench$ind$Domain)

## Create the mapping table based on the order obtained above
map_tab <- data.frame(pop_data_id = fh_bench$ind$Domain[domain_ord],
                      shape_id = shp_dt[[area_vars[1]]])

## Create map
map_plot(object = fh_bench, MSE = TRUE, map_obj = shp_dt,
